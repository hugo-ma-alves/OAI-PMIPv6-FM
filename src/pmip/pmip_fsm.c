...
int mag_fsm(msg_info_t * info)
{
  ...
  //Check if this PBU is for me, this is only necessary when there is more than
  //one MAg in the same machine
  if(info->msg_event == hasPBA && !IN6_ARE_ADDR_EQUAL(&info->dst,&conf.MagAddressEgress[0])){
    dbg("Received a PBA that wasn't for me. It was for %x:%x:%x:%x:%x:%x:%x:%x I am %x:%x:%x:%x:%x:%x:%x:%x \n"
      ,NIP6ADDR(&info->dst) , NIP6ADDR(&conf.MagAddressEgress[0]) );
    return result;
  }
  ...
  struct in6_addr hw_address = EUI64_to_EUI48(info->mn_iid);
    //struct in6_addr serv_mag_addr = info->mn_serv_mag_addr;
  struct in6_addr serv_mag_addr = conf.OurAddress;

  int type = pmip_cache_exists(&info->mn_nai, &serv_mag_addr);
  int mutex_return_code;
  
  ip6mn_nai_t mn_nai;
  memcpy(&mn_nai,&info->mn_nai,sizeof(ip6mn_nai_t));

  dbg("Entering in mag_fsm with the NAI: ");
  dbg_buf_string(&mn_nai, sizeof(ip6mn_nai_t));
  dbg("\n\n");
  ...
  switch (type) {
    ...
    if (info->iif == g_ingress_iif) {
      if (info->msg_event == hasRS) {
        ...
        prefix = mnid_hnp_map(hw_address, &mn_nai ,&aaa_result);
        if (aaa_result >= 0) {
          bce->mn_nai = mn_nai;
    ...
    if (info->msg_event == hasPBA) {
      bce = pmip_cache_get(&info->mn_nai, &serv_mag_addr);
      if (is_pba_is_response_to_last_pbu_sent(info, bce)) {
        ...
        //--------------------------------------
            ...
            if (info->msg_event == hasRS) {
              ...
              bce = pmip_cache_get(&info->mn_nai, &hw_address);
...
//---------------------------------------------------------------------------------------------------------------------
      int lma_fsm(msg_info_t * info)
      {
        ...
        int type = pmip_cache_exists(&mn_nai, &serv_mag_addr);
        switch (type) {
    //--------------------------------------
          case BCE_NO_ENTRY:
          ...
          if (info->msg_event == hasPBU && (info->lifetime.tv_sec > 0)) {
            ...
            if (bce != NULL) {
              pmip_insert_into_hnp_cache(hw_address, info->mn_prefix,mn_nai);
        ...
        //--------------------------------------
            ...
            if (IN6_ARE_ADDR_EQUAL(&info->src, &bce->mn_serv_mag_addr) ) //Received PBU from same MAG as before
            {
              lma_update_binding_entry(bce, info);
              lma_reg_no_new_tunnel (bce);
            } else {                                                      //Received PBU from an new MAG
              dbg("Handover detected\n");
              dbg("Deregistering from previously registered MAG\n");
