...
int flowMarker_add_fwmark(struct in6_addr ip_source,struct in6_addr ip_destination,
	...
	sprintf(buff, "nft insert rule ip6 pmip prerouting ip6 saddr %x:%x:%x:%x:%x:%x:%x:%x/128 ip6 daddr %x:%x:%x:%x:%x:%x:%x:%x/128  %s dport %d %s sport %d  meta mark set %d return",
		NIP6ADDR(&ip_source),NIP6ADDR(&ip_destination),
		transport_protocol_string, transport_protocol_destination_port,transport_protocol_string,
		transport_protocol_source_port, mark	);
	if(ret != 0){
		ret = system(buff);	
	}
