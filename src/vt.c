...
static int pmip_cache_vt_dump(void *data, void *arg)
{
	...
	if(&bce->mn_nai){
		...
		fprintf_bl(vh,"Client NAI:");
		fprintf_b(vh, " %s\n",nai);
	}

	fprintf_b(vh, "Peer_addr: ");
	fprintf(vh->vh_stream, " %x:%x:%x:%x:%x:%x:%x:%x\n",NIP6ADDR(&bce->mn_addr) );
	...
	if (is_ha()) {
		fprintf_b(vh, "Serv_MAG_addr: ");
		fprintf(vh->vh_stream, "%x:%x:%x:%x:%x:%x:%x:%x\n",
			NIP6ADDR(&bce->mn_serv_mag_addr));
	}

	if (is_mag()) {
		fprintf_b(vh, "LMA_addr: ");
		fprintf(vh->vh_stream, " %x:%x:%x:%x:%x:%x:%x:%x\n",
			NIP6ADDR(&bce->mn_serv_lma_addr));
	}

	fprintf_b(vh, "Local Address: ");
	fprintf(vh->vh_stream, " %x:%x:%x:%x:%x:%x:%x:%x\n",
		NIP6ADDR(&bce->our_addr));
...
//Defined for FM-PMIP
/////////////////////////////////////////////////
struct fmpmip_flowmob_cache_vt_arg {
	const struct vt_handle *vh;
};

static int fmpmip_flowmob_cache_vt_dump(void *data, void *arg)
{
	fmpmip_flow_cache_entry *fce = (fmpmip_flow_cache_entry *)data;
	struct fmpmip_flowmob_cache_vt_arg *bva = (struct fmpmip_flowmob_cache_vt_arg *)arg;
	const struct vt_handle *vh = bva->vh;
	struct timespec ts_now;
...
static int fmpmip_flowmob_cache_vt_cmd_fmbc(const struct vt_handle *vh, const char *str)
{
	struct fmpmip_flowmob_cache_vt_arg bva;
	bva.vh = vh;
...
static struct vt_cmd_entry vt_cmd_fmc = {
	.cmd = "fmpmip",
	.parser = fmpmip_flowmob_cache_vt_cmd_fmbc,
};
////////////////////////////////////////////////////////////


//////////////////////////////////////////////////
//Defined for Client ROute
/////////////////////////////////////////////////
struct fmpmip_client_routes_vt_arg {
	const struct vt_handle *vh;
};

static int fmpmip_client_routes_vt_dump(void *data, void *arg)
{	
	struct fmpmip_client_routes_vt_arg *bva = (struct fmpmip_client_routes_vt_arg *)arg;
	const struct vt_handle *vh = bva->vh;

	struct route *r = (struct route *)data;
	...
	char nai[naiSize+1];
	nai[naiSize]='\0';
	memcpy(&nai,&r->client_nai,naiSize);
	fprintf_bl(vh,"Client(NAI): %s\n", nai);

	struct mag *m = r->mag;
	while(m!=NULL) {
		fprintf(vh->vh_stream, "--> MAG: %x:%x:%x:%x:%x:%x:%x:%x",NIP6ADDR(&m->ip6_addr));
		fprintf(vh->vh_stream, " || MARK: %d\n",m->mark );
		m= m->next;
	}
...
static int fmpmip_client_routes_vt_cmd_fmbc(const struct vt_handle *vh, const char *str)
{
	struct fmpmip_client_routes_vt_arg bva;
	bva.vh = vh;
...
static struct vt_cmd_entry vt_cmd_client_route_cmd = {
	.cmd = "routes",
	.parser = fmpmip_client_routes_vt_cmd_fmbc,
};
////////////////////////////////////////////////////////////
...
{
	int ret;
	ret = vt_cmd_add_root(&vt_cmd_fmc);
	if (ret < 0)
		return ret;
...
//Defined for clients routes////////////////
int vt_client_route_init(void)
{
	int ret;
	ret = vt_cmd_add_root(&vt_cmd_client_route_cmd);
	if (ret < 0)
		return ret;
