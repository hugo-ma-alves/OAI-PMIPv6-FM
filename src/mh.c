...
#define __MH_SENTINEL (IP6_MH_TYPE_MAX + 1)
 ...
 static void *mh_listen(__attribute__ ((unused)) void *arg)
 {
 	...
 	pthread_dbg("Mobility header thread started");
 ...
 int mh_init(void)
 {
	...
	//If it is a MAG bind the socket to the interface connected to thg LMA
	//If it is a LMA bind the socket to the interface connected to thg MAG
 	//if (is_mag()) {
 		//setsockopt(mh_sock.fd, SOL_SOCKET, SO_BINDTODEVICE, conf.MagDeviceEgress,  strlen(conf.MagDeviceEgress));
 		//dbg("Binding Mobility header raw socket to %s interface\n",conf.MagDeviceEgress );
 	//}else if(is_ha()){
 		//setsockopt(mh_sock.fd, SOL_SOCKET, SO_BINDTODEVICE, conf.LmaPmipNetworkDevice,  strlen(conf.LmaPmipNetworkDevice));
 		//dbg("Binding Mobility header raw socket to %s interface\n",conf.LmaPmipNetworkDevice );
 	//}
